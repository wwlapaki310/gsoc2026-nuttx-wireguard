...
| 2 | **Summary** — NuttX に WireGuard を実装した | WireGuard の説明 + 構成図 | 60 秒 |
| 3 | **Motivation** — NuttX に、リモートからセキュアにアクセスしたい | 2 つの現場の表 | 100 秒 |
...
Apache NuttX 上で動く WireGuard VPN の実装。`wg0` というネットワークデバイスとして見える。
<sub>README.ja.md 冒頭</sub>

秋田 賢 / ソニーセミコンダクタソリューションズ
...
## 2 — Summary: NuttX に WireGuard を実装した

**WireGuard とは**（README.ja.md「プロジェクト概要」）

| | |
|---|---|
| 何 | Linux 向けに開発された軽量な VPN プロトコル。組み込み・IoT 分野でも採用が広がっている |
| 中身 | UDP 上で暗号化トンネルを確立する。Curve25519・ChaCha20-Poly1305・BLAKE2s |
| 規模 | 実装がコンパクト（約 4,000 行）で、マイコン上での動作にも適している |

> WireGuard を NuttX のネットワークデバイス（`wg0`）として実装した。

**図案:** README.ja.md「アーキテクチャ」の構成図。アプリケーション / NSH → NuttX ネットワーク
スタック（BSD socket API）→ `eth0 / wlan0` と並んで `wg0`（WireGuard netdev）→ UDP ソケット
（ポート 51820）→ インターネット / LTE / 衛星回線 → WireGuard ピア（Linux, Windows, ...）。

**話で補う:** Apache NuttX は POSIX 準拠の RTOS で、独自の TCP/IP スタックと BSD ソケット API を
持つが、現時点では VPN 機能が存在しない。通信相手は既存の WireGuard エンドポイントで構わない。

---

## 3 — Motivation: NuttX に、リモートからセキュアにアクセスしたい

| | 何 | ボード / OS |
|---|---|---|
| **AITRIOS** | エッジ AI プラットフォームの AI カメラ | **ESP32 + NuttX** |
| **人工衛星** | ボトムアップ活動のプロジェクト | **SPRESENSE** |

> ### NuttX には現時点で VPN 機能が存在しない。
> VPN がない場合の現実的な選択肢は、グローバル IP を晒すか、独自プロトコルを作り込むか、
> ベンダーのクラウドに乗るか——どれも嬉しくない。（README.ja.md）

**話で補う（スライドに書かない）:** きっかけは GSoC のテーマ一覧でこの題目を見つけたこと。
いずれも NuttX の機器で、リモートからセキュアにアクセスしたいと思っていた。NuttX デバイスへの
遠隔・安全なアクセスは多くの分野で未解決の課題（README.ja.md）。WireGuard は Tailscale で
使われているプロトコルで、自分は Tailscale のヘビーユーザーだった。

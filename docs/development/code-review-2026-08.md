...
> **【2026-08-30 追記】この結論は誤りだった。**
>
> 上では「2回の独立した計測でピークが 2,896 B で一致しており再現性のある値」と書いたが、
> 一致していたのは**同じ短時間バーストを2回流したから**であって、最悪ケースを捉えていた
> わけではなかった。後日ボードを数時間動かしたあとに取れたクラッシュダンプでは、
> `wg_rx` の高水位が **3,392 B (83.6%)** に達していた([phase4-log.md](phase4-log.md) 参照)。
> 4096 では余裕が 704 バイトしかない。
>
> 深いパスは「パケットがどの状態の組み合わせで到着するか」に依存するため、
> 短時間の負荷試験は最悪ケースにならない。
> 既定値は **6144** に引き上げ済み(3,392 B に対して約 45% の余裕)。
> Kconfig の help にも「短時間の計測を信用しないこと」を明記した。
